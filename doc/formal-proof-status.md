...
The original generalized-graph enumeration built 1,239 generated
`R(4,4,k)` theory scripts for `k=8..17` in 7:26:35.  The subsequent one-core
`enumf` merge ran for 23:47.91 and then failed at `R4418` with HOL's
`end_itlist` / `list too short` exception.  Source and artifact inspection
showed that upstream's commented generation command uses the half-open range
`range (8,18,...)`: it omitted the required order-18 batch generated from the
unique order-17 critical graph.  Consequently `collect_44k 18` was empty.
This was a missing generated theory, not an empty mathematical case and not a
resource failure.

The corrected enumeration family contains 1,240 theories.  The missing
`ramseyEnum4418_0Script.sml` has been generated by the upstream HOL function
and binds one `NEXT_R_THM_ONE 18 (4,4)` theorem.  Its first kernel build
exported `R4418_0`; a clean second build under the original 8,000 MB
enumeration configuration also exited zero in 2:19.77 with 3,684,876 KiB peak
RSS.  The corrected 1,240-theory five-artifact audit passed, and its manifest
hashes to
