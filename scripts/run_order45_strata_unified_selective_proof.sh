...
proof_dir=${2:-"$root/build/order45-strata/unified-selective-b30000-c128000-la1-primary0-solve10"}
...
"$runner" \
  ...
  "$stem.drat" "$stem.tsv" 30000 128000 1 0 10 \
  > "$stem.log" 2>&1
runner_status=$?
set -e
...
python3 "$root/tools/audit_order45_strata_proofs.py" \
  ...
  --maximum-primary-split-variable 0 \
